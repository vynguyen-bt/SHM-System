...
// Hàm kiểm tra CSS variables consistency
function checkCSSVariables() {
  ...
  return variables.every(v => rootStyles.getPropertyValue(v).trim());
}

// Hàm so sánh trước và sau khi cập nhật
function compareUIChanges() {
  console.log('\n📊 === SO SÁNH THAY ĐỔI UI ===');

  const report = checkUIConsistency();
  const hasVariables = checkCSSVariables();

  console.log('\n✅ IMPROVEMENTS:');
  console.log(`  CSS Variables: ${hasVariables ? 'Implemented' : 'Not found'}`);
  console.log(`  Inconsistencies: ${report.inconsistencies.length} issues`);

  if (report.inconsistencies.length === 0) {
    console.log('\n🎉 UI CONSISTENCY ACHIEVED!');
  } else {
    console.log('\n⚠️ Remaining issues to fix:');
    report.inconsistencies.forEach((issue, i) => {
...
if (typeof window !== 'undefined') {
  ...
  window.compareUIChanges = compareUIChanges;
}
